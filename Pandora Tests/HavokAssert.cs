...
public static class HavokAssert
{
    ...
    private static readonly List<System.Type> types = new() { typeof(hkbStateMachineStateInfo) };
    ...
    {
        return type.IsAssignableFrom(typeof(IList<>))
            || type.IsAssignableFrom(typeof(IEnumerable<>))
            || type.GetInterfaces().Any(i => IsList(i));
    }
    ...
    public static void Valid(IHavokObject havokObject, HashSet<IHavokObject> visited)
    {
        ...
        var isList = new Predicate<System.Type>(i =>
        ...
        foreach (var propertyInfo in havokObject.GetType().GetProperties())
        {
            ...
            if (
                propertyType == typeof(IHavokObject)
                || propertyType.GetInterfaces().Any(i => i.IsAssignableFrom(typeof(IHavokObject)))
            )
            {
                IHavokObject? propertyValue = (IHavokObject)
                    accessor[havokObject, propertyInfo.Name];
                var nullable =
    ...
    public static void NotNullValid(IHavokObject? referencedObject)
    {
        NotNullValid(
            referencedObject,
            new HashSet<IHavokObject>(ReferenceEqualityComparer.Instance)
        );
    }
}
