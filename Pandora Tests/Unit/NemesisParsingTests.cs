...
namespace PandoraTests.Unit
{
	public class NemesisParsingTests
	{
		...
		[Fact]
		public void ParseInsertEdit_WhenEditTextNoNextSibling_AddsAppendTextChange()
		{
			string nodeName = "#0069";
			var element = new XElement(
				"property",
		...
		[Fact]
		public void ParseInsertEdit_WhenEditTextNextSiblingIsElement_AddsAppendTextChange()
		{
			string nodeName = "#0069";
			var element = new XElement(
				"property",
				...
				new XText("NewText"),
				new XComment("CLOSE"),
				new XElement("sibling")
			);
			var children = element.Nodes().ToList();
			var match = new XMatch([children[0], children[1], children[2]]);
			IPackFileChangeOwner changeSet = Substitute.For<IPackFileChangeOwner>();
			var lookup = Substitute.For<IXPathLookup>();
			NemesisParser.ParseInsertEdit(nodeName, match, changeSet, lookup);
			changeSet.Received().AddChange(Arg.Any<AppendTextChange>());
		...
		[Fact]
		public void ParseInsertEdit_WhenMultipleElements_AddsMultipleAppendChanges()
		{
			string nodeName = "#0069";
			var match = new XMatch([
				new XComment("OPEN"),
			...
			lookup.LookupPath(Arg.Any<XNode>()).Returns("#0069/property");
			NemesisParser.ParseInsertEdit(nodeName, match, changeSet, lookup);
			changeSet.Received(3).AddChange(Arg.Any<AppendElementChange>());
		}

		[Fact]
		public void ParseReplaceEdit_WhenMultipleElementReplacements_AddsMultipleReplaceChanges()
		{
			string nodeName = "#0069";
			var match = new XMatch([
				new XComment("OPEN"),
				...
				new XElement("NewElement2"),
				new XComment("ORIGINAL"),
				new XElement("OldElement1"),
				new XElement("OldElement2"),
				new XComment("CLOSE"),
			]);
			IPackFileChangeOwner changeSet = Substitute.For<IPackFileChangeOwner>();
			var lookup = Substitute.For<IXPathLookup>();
		...
		[Fact]
		public void ParseReplaceEdit_WhenMultipleTextReplacements_AddsMultipleReplaceChanges()
		{
			string nodeName = "#0069";
			var match = new XMatch([
				new XComment("OPEN"),
			...
			var lookup = Substitute.For<IXPathLookup>();
			NemesisParser.ParseReplaceEdit(nodeName, match, changeSet, lookup);
			changeSet.Received(2).AddChange(Arg.Any<ReplaceTextChange>());
		}

		[Fact]
		public void ParseReplaceEdit_WhenNoNewElements_MultipleOldElements_AddsMultipleRemoveChanges()
		{
			string nodeName = "#0069";
			var match = new XMatch([
				new XComment("OPEN"),
				...
				new XElement("OldElement1"),
				new XElement("OldElement2"),
				new XElement("OldElement3"),
				new XComment("CLOSE"),
			]);
			IPackFileChangeOwner changeSet = Substitute.For<IPackFileChangeOwner>();
			var lookup = Substitute.For<IXPathLookup>();
			NemesisParser.ParseReplaceEdit(nodeName, match, changeSet, lookup);
			changeSet.Received(3).AddChange(Arg.Any<RemoveElementChange>());
		}

		[Fact]
		public void MatchReplacePattern_WhenNoMatches_ReturnsFalse()
		{
			string nodeName = "#0069";
			var nodes = new List<XNode> { new XElement("plain"), new XText("nothing special") };
			IPackFileChangeOwner changeSet = Substitute.For<IPackFileChangeOwner>();
			var lookup = Substitute.For<IXPathLookup>();
			bool result = NemesisParser.MatchReplacePattern(nodeName, nodes, changeSet, lookup);
			Assert.False(result);
			changeSet.DidNotReceive().AddChange(Arg.Any<IPackFileChange>());
		}

		[Fact]
		public void MatchInsertPattern_WhenNoMatches_ReturnsFalse()
		{
			string nodeName = "#0069";
			var nodes = new List<XNode> { new XElement("plain"), new XText("nothing special") };
			var modInfo = Substitute.For<IModInfo>();
