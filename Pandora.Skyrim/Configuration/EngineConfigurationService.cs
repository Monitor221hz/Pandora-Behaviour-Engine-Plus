...
public sealed class EngineConfigurationService : IEngineConfigurationService
{
	...
	public void Initialize(IEngineConfiguration startupConfig)
	{
		...
		{
			SetCurrentFactory(startupFactory);
		}
	}

	private IEngineConfigurationFactory? ResolveFactoryFor(IEngineConfiguration config)
	{
		var configType = config.GetType();

		if (_skyrimFactory.Create()?.GetType() == configType)
			return _skyrimFactory;

		if (_skyrimDebugFactory.Create()?.GetType() == configType)
			return _skyrimDebugFactory;
