...
public class JsonPluginLoader : IMetaPluginLoader
{
	...
	public bool TryLoadMetadata(
		...
		[NotNullWhen(true)] out IPluginInfo? pluginInfo
	)
	{
		pluginInfo = null;
		FileInfo infoFile = new(
			Path.Join(directory.FullName, string.Concat(IPluginInfo.FILE_HEADER, ".json"))
		);
		if (!infoFile.Exists)
		{
			return false;
	...
	{
		string pluginPath = Path.IsPathRooted(pluginInfo.Path)
			? pluginInfo.Path
			: Path.Join(directory.FullName, pluginInfo.Path);
