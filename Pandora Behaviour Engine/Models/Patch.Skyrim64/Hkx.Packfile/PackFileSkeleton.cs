...
public class PackFileSkeleton : PackFile, IPackFileSkeleton
{
	...
	public PackFileSkeleton(FileInfo file)
		: this(file, null) { }
	...
	[MemberNotNull(
		nameof(MergedAnimationContainer),
		nameof(PhysicsData),
		nameof(RagdollInstance),
		nameof(SkeletonMappers),
		nameof(ResourceContainer)
	)]
	public override void Load()
	{
		...
		{
			if (namedVariant == null || namedVariant.variant == null)
			{
				continue;
			}
	...
	public override void PopPriorityXmlAsObjects()
	{
		...
		if (HasResourceContainer)
			PopObjectAsXml(ResourceContainer);
		if (HasPhysicsData)
			PopObjectAsXml(PhysicsData);
		if (HasRagdollInstance)
	...
	public override void PushPriorityObjects()
	{
		...
		if (HasResourceContainer)
			PushXmlAsObject(ResourceContainer);
		if (HasPhysicsData)
