...
public class AnimSet
{
	...
	public static bool TryRead(StreamReader reader, [NotNullWhen(true)] out AnimSet? animSet)
	{
		...
		if (!int.TryParse(reader.ReadLineOrEmpty(), out int numTriggers))
		{
			return false;
		}
		...
		if (!int.TryParse(reader.ReadLineOrEmpty(), out int numConditions))
		{
			return false;
		}
		...
		if (!int.TryParse(reader.ReadLineOrEmpty(), out int numAttacks))
		{
			return false;
		}
		...
		if (!int.TryParse(reader.ReadLineOrEmpty(), out int numAnimationInfos))
		{
			return false;
		}
	...
	public override string ToString()
	{
		...
		if (NumAttackEntries > 0)
		{
			sb.AppendJoin("\r\n", AttackEntries).AppendLine();
		}

		sb.AppendLine(NumAnimationInfos.ToString());
		if (NumAnimationInfos > 0)
		{
			sb.AppendJoin("", AnimInfos);
		}
