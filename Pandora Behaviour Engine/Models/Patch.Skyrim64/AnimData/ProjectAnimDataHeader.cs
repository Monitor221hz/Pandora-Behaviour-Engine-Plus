...
namespace Pandora.Models.Patch.Skyrim64.AnimData
{
	...
	{
		public ProjectAnimDataHeader(
			int leadInt,
			int assetCount,
			IList<string> projectAssets,
			int hasMotionData
		)
		{
			LeadInt = leadInt;
		...
		public static bool TryReadBlock(
			StreamReader reader,
			[NotNullWhen(true)] out ProjectAnimDataHeader? header
		)
		{
			header = null;
			if (
				!int.TryParse(reader.ReadLine(), out var leadInt)
				|| !int.TryParse(reader.ReadLine(), out var assetCount)
			)
			{
				return false;
			}
			...
			if (!int.TryParse(reader.ReadLine(), out var hasMotionData))
			{
				return false;
			}
		...
		public override string ToString()
		{
			...
			sb.AppendLine(LeadInt.ToString())
				.AppendLine(ProjectAssets.Count.ToString())
