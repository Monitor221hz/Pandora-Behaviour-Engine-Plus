...
public class PackFile : IEquatable<PackFile>
{
    ...
    public PackFile(FileInfo file, Project? project)
    {
        InputHandle = file;
        OutputHandle = file;
        relativeOutputFilePath = Path.GetRelativePath(BehaviourEngine.AssemblyDirectory.FullName, file.FullName.Replace(Path.Combine("Pandora_Engine", "Skyrim", "Template"), "meshes", StringComparison.OrdinalIgnoreCase));
