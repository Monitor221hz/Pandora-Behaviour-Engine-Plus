...
namespace Pandora.Core.Patchers.Skyrim
{
	public class ProjectManager : IProjectManager
	{
		...
		{
			var fileInfo = BehaviourEngine.SkyrimGameDataDirectory != null ? packFile.GetRebasedOutput(BehaviourEngine.SkyrimGameDataDirectory) : packFile.OutputHandle; 
			if (!fileInfo.Exists)
			{
				outPackFile = default;
				return false;
		...
		{
			var fileInfo = new FileInfo(Path.ChangeExtension((BehaviourEngine.SkyrimGameDataDirectory != null ? packFile.GetRebasedOutput(BehaviourEngine.SkyrimGameDataDirectory) : packFile.OutputHandle).FullName, extension));
			if (!fileInfo.Exists)
			{
				outPackFile = default;
				return false;
