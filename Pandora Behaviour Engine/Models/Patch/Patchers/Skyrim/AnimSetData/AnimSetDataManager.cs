...
namespace Pandora.Patch.Patchers.Skyrim.AnimSetData
{
    public class AnimSetDataManager
    {
        ...
        public AnimSetDataManager(DirectoryInfo templateFolder, DirectoryInfo meshDirectory)
        {
            ...
            templateAnimSetDataSingleFile = new FileInfo(Path.Combine(templateFolder.FullName, ANIMSETDATA_FILENAME));
            vanillaHkxFiles = new FileInfo(Path.Combine(templateFolder.FullName, "vanilla_hkxpaths.txt"));

            this.outputMeshDirectory = meshDirectory;
            outputAnimSetDataSingleFile = new FileInfo(Path.Combine(meshDirectory.FullName, ANIMSETDATA_FILENAME));
        }

        public void SetOutputPath(DirectoryInfo meshDirectory)
        {
            this.outputMeshDirectory = meshDirectory;
            outputAnimSetDataSingleFile = new FileInfo(Path.Combine(outputMeshDirectory.FullName, ANIMSETDATA_FILENAME));
        }
            ...
            using (var readStream = templateAnimSetDataSingleFile.OpenRead())
            {
                using (var reader = new StreamReader(readStream))
                {
                    ...
                    for (int i = 0; i < NumProjects; i++)
                    {
                        ...
                        //#if DEBUG
                        //                      FileInfo animDataFile = new FileInfo(Path.Combine(outputFolder.FullName, "animsetdata", Path.GetFileName(projectPaths[i])));
