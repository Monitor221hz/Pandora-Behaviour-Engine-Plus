...
public static class ModUtils
{
	...
	public static bool IsPandoraMod(string? code) =>
	...
	public static void NormalizeModPriorities(IEnumerable<ModInfoViewModel> mods)
	{
		...
		foreach (
			var mod in mods.Where(mod => !IsPandoraMod(mod))
	...
	public static void SetAlphanumericPriorities(IEnumerable<ModInfoViewModel> mods)
	{
		...
		foreach (
			var mod in mods.Where(m => !IsPandoraMod(m))
	...
	public static ModInfoViewModel? EnsurePandoraModActive(
		IEnumerable<ModInfoViewModel> mods,
		uint? priority = null
	)
	{
	...
	public static void ApplySortToColumn(
		DataGridColumnHeader? header,
		Action<DataGridColumn> sortAction
	)
	{
		if (header == null)
			return;
